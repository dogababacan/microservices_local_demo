...
async function connectWithRetry() {
  while (true) {
    try {
      ...
      console.error("[Notification Service] RabbitMQ not ready yet. Retrying in 5 seconds...");
      await wait(5000);
    }
  }
}
...
async function consumeEvents(queueName, routingKeys, handler) {
  ...
  for (const routingKey of routingKeys) {
    // bindQueue subscribes the same service inbox to each note label it needs.
  ...
  channel.consume(queueName, async (msg) => {
    ...
    try {
      ...
      console.error("[Notification Service] Invalid JSON message", error.message);
    ...
    try {
      ...
      console.error("[Notification Service] Error while processing message", error.message);
