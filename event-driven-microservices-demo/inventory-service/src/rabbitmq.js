...
async function connectWithRetry() {
  while (true) {
    try {
      console.log("[Inventory Service] Connecting to RabbitMQ...");
      ...
      console.error("[Inventory Service] RabbitMQ not ready yet. Retrying in 5 seconds...");
      await wait(5000);
    }
  }
}
...
async function consumeEvent(queueName, routingKey, handler) {
  ...
  channel.consume(queueName, async (msg) => {
    ...
    try {
      ...
      console.error("[Inventory Service] Invalid JSON message", error.message);
    ...
    try {
      ...
      console.error("[Inventory Service] Error while processing message", error.message);
